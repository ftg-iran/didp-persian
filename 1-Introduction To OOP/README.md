...
رابط ها و کلاس/متود های انتزاعی اکثر زبان های برنامه نویسی بر پایه مفاهیم انتزاعی و کپسوله سازی هستند. در زبان های برنامه نویسی شیء گرا مدرن، مکانیزم رابط(معمولا با کلمه کلیدی `interface`  یا `protocol` تعریف میشود) به شما این امکان را میدهد که قواعد تعامل بین اشیاء را تعریف کنید.  این یکی از دلایلی است که  رابط ها فقط  به رفتار اشیاء  اهمیت میدهند و اینکه چرا نمیتوانید  یک فیلد در رابط ها  تعریف کنید.
این واقعیت که کلمه رابط مخفف بخش عمومی یک شیء است، در حالی که نوع `interface` در اکثر زبان های برنامه نویسی نی وجود دارد، بسیار گیج کننده است و من در این مورد با شما موافقم.
تصور کنید رابط `Flying Transport` همرا با متود `fly(origin, destination, passengers)` را دارید. هنگامی طراحی یک شبیه ساز حمل و نقل هوایی، میتوانید کلاس `Airport` را محدود کنید تا فقط با اشیایی که `FlyingTransport` را پیاده سازی میکنند کار کند. سپس، مطمئن خواهید بود که هر شیء  داده شده به شیء هواپیما، چه `Airplane` ، چه `Helicopter`  و چه یک `DomesticatedGryphon` خواهد توانست در این نوع فرودگاه، فرود بیاید یا از آن خارج شود.
