...
الگوی متود کارخانه پیشنهاد می‌کند که تماس‌های ساخت مستقیم شی (با استفاده از اپراتور جدید) را با فراخوانی به متود کارخانه خاص جایگزین کنید. نگران نباشید؛ اشیاء هنوز از طریق اپراتور جدید ایجاد می شوند، اما از متود کارخانه فراخوانی می شوند. اشیایی که با متود کارخانه ای بازگردانده می شوند اغلب به عنوان محصولات شناخته می شوند.
...
کدی که از متود کارخانه استفاده می کند (اغلب کد مشتری[^4] نامیده می شود) تفاوتی بین محصولات واقعی بازگردانده شده توسط زیر کلاس های مختلف نمی بیند. مشتری با تمام محصولات به عنوان `Transport` انتزاعی برخورد می کند.

![Truck](./images/diagrams/factory-method/solution3-en.png)

تا زمانی که همه کلاس‌های محصول یک رابط مشترک را پیاده‌سازی کنند، می‌توانید اشیاء آنها را بدون شکستن آن به کد مشتری منتقل کنید.

مشتری می داند که قرار است همه اشیاء حمل و نقل دارای متود `deliver` باشند، اما نحوه عملکرد دقیق آن برای مشتری مهم نیست.
...
برای اینکه این الگو کار کند، کلاس محاوره ای پایه باید با دکمه های انتزاعی کار کند: یک کلاس پایه یا یک رابط که همه دکمه های بتنی از آن پیروی می کنند. به این ترتیب کد دیالوگ، با هر نوع دکمه ای که کار می کند، کاربردی باقی می ماند.
...
وراثت احتمالاً ساده ترین راه برای گسترش رفتار پیش فرض یک کتابخانه یا چارچوب است. اما چارچوب چگونه تشخیص می دهد که زیر کلاس شما باید به جای یک جزء استاندارد استفاده شود؟
